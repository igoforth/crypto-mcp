/**
 * Crypto Wallet Inspector — Tool Registration
 *
 * Export this to extend the server with additional tools.
 */
...
/**
 * Register all crypto wallet inspector tools on the given MCP server.
 */
		...
		async ({ address, token, chain, rpcUrl }) => {
			...
			if (token === "all") {
				const balances: Record<
					string,
					{ balance: string; raw: string; decimals: number; address?: string }
				> = {};
				...
				return formatResponse({ address, chain, balances });
			}
